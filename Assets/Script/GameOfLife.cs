...
{
    [SerializeField] private TextAsset _stageTextAsset;
    ...
    void LoadCellStatus()
    {
        ...
        {
            this._cellStatusArray[0,j] = new CellStatus(false,true,false);
        }

        for(int i=0; i<this._cellStatusArray.GetLength(0) - 2; i++)
        {
            this._cellStatusArray[i + 1, 0] = new CellStatus(false,true,false);
            ...
            for(int j=0; j<this._cellStatusArray.GetLength(1) - 2; j++)
            {
                switch(words[j].Trim())
                {
                    case "0": this._cellStatusArray[i + 1, j +1] = new CellStatus(false,false,false);
                    break;

                    case "1": this._cellStatusArray[i + 1, j +1] = new CellStatus(true,false,false);
                    break;

                    case "2": this._cellStatusArray[i + 1, j +1] = new CellStatus(false,true,false);
                    break;

                    case "3": this._cellStatusArray[i + 1, j +1] = new CellStatus(true,true,false);
                    break;

                    case "4": this._cellStatusArray[i + 1, j +1] = new CellStatus(false,true,true);
                    break;

                    default: this._cellStatusArray[i + 1, j +1] = new CellStatus(false,true,false);
                    break;
                }
            }

            this._cellStatusArray[i + 1, this._cellStatusArray.GetLength(1) - 1] = new CellStatus(false,true,false);
        }

        for(int j=0; j<this._cellStatusArray.GetLength(1); j++)
        {
            this._cellStatusArray[this._cellStatusArray.GetLength(0) - 1, j] = new CellStatus(false,true,false);
        }
